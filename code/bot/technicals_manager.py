...
def process_candles(df: pd.DataFrame, pair, trade_settings: TradeSettings, log_message, log_error, trade_risk):
...
def fetch_candles(pair, row_count, candle_time, granularity, 
    ...
    if df is None or df.shape[0] == 0:
        log_message("tech_manager fetch_candles failed to get_candles", pair)
        return None
